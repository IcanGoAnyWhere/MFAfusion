...
def sample_points_with_roi_square(rois, points, sample_radius_with_roi, num_max_points_of_part=200000):
    """
    Args:
        rois: (M, 7 + C)
        points: (N, 3)
    ...
    distance_3d = torch.abs((points[:, None, :] - rois[None, :, 0:3]))
    delta = distance_3d < rois[None, :, 3:6] / 2 + 0.5
    mask = torch.sum(delta, dim=-1)
    mask = mask == 3
    mask = torch.sum(mask, dim=-1)
    square_mask = mask > 0

    sampled_points = points[:1] if square_mask.sum() == 0 else points[square_mask, :]

    return sampled_points, square_mask
...
class VoxelSetAbstraction(nn.Module):
    ...
    def get_sampled_points(self, batch_dict):
        ...
        for bs_idx in range(batch_size):
            ...
            elif self.model_cfg.SAMPLE_METHOD == 'PAS':

                cur_keypoints, square_mask = sample_points_with_roi_square(
                    rois=batch_dict['rois'][bs_idx], points=sampled_points[0],
                    sample_radius_with_roi=self.model_cfg.SPC_SAMPLING.SAMPLE_RADIUS_WITH_ROI,
                    num_max_points_of_part=self.model_cfg.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000)
                )
                bs_idxs = cur_keypoints.new_ones(cur_keypoints.shape[0]) * bs_idx
        ...
        # visualize_point = keypoints[:, 1:].cpu().numpy()
    ...
    def forward(self, batch_dict):
        ...
        # visualize_point = keypoints[:, 1:4].cpu().numpy()
        # pcd_point = o3d.geometry.PointCloud()
        # pcd_point.points = o3d.utility.Vector3dVector(visualize_point)
        # o3d.visualization.draw_geometries([pcd_point], window_name='sumlable',
        #                                   width=1024, height=768,
